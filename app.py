    ...
    if st.session_state.get('hesaplama_tamam', False):
        ...
        # --- YENİ YERİ: HESAPLAMALARIN EN ALTINA ---
            ...
            try:
                ...
                pdf.set_font(f_isim, size=11, style='')
                pdf.multi_cell(190, 6, txt=f"{str(company_name)} olarak raporlama yılında Mavi su olarak; insani kullanım ve üretim amacıyla şebeke suyu, kuyu suyu ve diğer tatlı su kaynakları temin edilmektedir. Mavi su ayak izi hesabında sayaç tüketimleri ve faturalar kabul edilerek hesap yapılmaktadir.\n\nGri su olarak; üretim amacıyla proseste kullanılan suyun endüstriyel nitelikli atıksu faaliyeti sonucunda arıtma tesislerine veya kanalizasyona deşarjı baz alınmaktadır.")
                ...
                pdf.set_font(f_isim, size=10, style='')
                pdf.multi_cell(190, 6, txt="Tesisin doğrudan tükettiği, buharlastırdığı veya ürüne kattığı tatlı su miktarını temsil eder. Tesis giriş suyu ile deşarj arasındaki kütle denkliğine göre hesaplanmıştır.")
                ...
                pdf.set_fill_color(220, 220, 220)
                pdf.cell(95, 8, txt="Veri Kaynağı", border=1, fill=True, align='C')
                pdf.cell(95, 8, txt="Toplam Tuketim (m³/yıl)", border=1, ln=True, fill=True, align='C')
                pdf.set_font(f_isim, size=10, style='')
                ...
                pdf.set_font(f_isim, size=12, style='B')
                pdf.cell(190, 8, txt=f"Toplam Su Ayak İzi: {format_num_anlik(total_vol)} m³/yil", ln=True)
                pdf.ln(3)
